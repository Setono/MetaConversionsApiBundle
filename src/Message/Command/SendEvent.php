...
final class SendEvent implements CommandInterface
{
    ...
    public static function fromEvent(Event $event): self
    {
        return new self(
            ...
            $event->eventId,
            $event->getPayload(),
            // The client posts once per pixel, so a pixel that a provider listed twice must travel once
            array_values(array_unique(array_map(static fn (Pixel $pixel): string => $pixel->id, $event->pixels))),
            $event->testEventCode,
        );
    }
}
