...
final class CookieBasedFbcContextTest extends TestCase
{
    ...
    #[Test]
    public function it_says_when_a_cookie_could_not_be_parsed(): void
    {
        $logger = $this->createMock(LoggerInterface::class);
        $logger->expects(self::once())->method('debug')->with(self::stringContains('could not be parsed'));

        (new CookieBasedFbcContext(self::requestStack('not-an-fbc-cookie'), $logger))->getFbc();
    }
    ...
    public function it_does_not_log_for_a_visitor_without_a_cookie(): void
    {
        ...
        (new CookieBasedFbcContext(self::requestStack(null), $logger))->getFbc();
    }
