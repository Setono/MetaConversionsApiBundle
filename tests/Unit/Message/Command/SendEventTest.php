...
final class SendEventTest extends TestCase
{
    ...
    #[Test]
    public function it_carries_each_pixel_once(): void
    {
        $event = self::event();
        $event->pixels[] = new Pixel('1234', 's3cr3t');

        // The client posts the event once per pixel it is handed
        self::assertSame(['1234'], SendEvent::fromEvent($event)->pixelIds);
    }
